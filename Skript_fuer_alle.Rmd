...
umsatz$Datum<-as.POSIXct.Date(umsatz$Datum)
kiwo <- read_csv("kiwo.csv", col_types = cols(Datum = col_date(format = "%Y-%m-%d")))
kiwo$Datum<-as.POSIXct.Date(kiwo$Datum)
wetter <- read_delim("Wetter_Trennzeichen geaendert.csv", 
    ";", escape_double = FALSE, col_types = cols(Datum = col_date(format = "%d.%m.%Y")), 
    trim_ws = TRUE)
wetter$Datum<-as.POSIXct.Date(wetter$Datum)
...
arbeitslosigkeit <- read_delim("Arbeitslosigkeit.csv", 
    ";", escape_double = FALSE, col_types = cols(Datum = col_date(format = "%d.%m.%Y")), 
    trim_ws = TRUE) 
arbeitslosigkeit$Datum<-as.POSIXct.Date(arbeitslosigkeit$Datum)

semesterferien <- read_excel("CAU_Semesterferien.xlsx", 
    col_types = c("date", "numeric"))
semesterferien$Datum<-as.POSIXct.Date(semesterferien$Datum)

datumsangaben <- read_delim("Date.csv", ";", escape_double = FALSE, 
    col_types = cols(Datum = col_date(format = "%d.%m.%Y")), 
    trim_ws = TRUE)
datumsangaben$Datum<-as.POSIXct.Date(datumsangaben$Datum)

digitale_woche <- read_excel("Digitale_Woche.xlsx", 
    col_types = c("date", "numeric"))
digitale_woche$Datum<-as.POSIXct.Date(digitale_woche$Datum)

feiertage <- read_delim("Feiertage.csv", 
    ";", escape_double = FALSE, col_types = cols(Datum = col_date(format = "%d.%m.%Y")))
feiertage <- feiertage[,-7] #so oft ausführen bis keine Zeile 7 mehr existiert
feiertage$Datum<-as.POSIXct.Date(feiertage$Datum)

jahreszeitenbeginn <- read_excel("Jahreszeitenbeginn.xlsx", 
    col_types = c("date", "numeric", "text"))
jahreszeitenbeginn$Datum<-as.POSIXct.Date(jahreszeitenbeginn$Datum)

jahreszeiten <- read_excel("Meteorologische Jahreszeiten.xlsx", 
    col_types = c("date", "numeric", "numeric", 
        "numeric", "numeric"))
jahreszeiten$Datum<-as.POSIXct.Date(jahreszeiten$Datum)

bevoelkerung <- read_excel("Population_Kiel.xlsx", 
    col_types = c("date", "numeric"))
bevoelkerung$Datum<-as.POSIXct.Date(bevoelkerung$Datum)

schulferien <- read_excel("Schulferien.xlsx", 
    col_types = c("date", "numeric"))
schulferien$Datum<-as.POSIXct.Date(schulferien$Datum)

temperaturdifferenz <- read_excel("Temperaturdifferenz_und_Durchschnitt.xlsx", 
    col_types = c("date", "numeric", "numeric", 
        "numeric", "numeric", "numeric"))
temperaturdifferenz$Datum<-as.POSIXct.Date(temperaturdifferenz$Datum)

tourismus <- read_excel("Tourismus.xlsx", 
    col_types = c("date", "numeric", "numeric", 
        "numeric", "numeric", "text"))
tourismus <- tourismus[,-6]
tourismus$Datum<-as.POSIXct.Date(tourismus$Datum)

wahlen <- read_excel("Wahlen.xlsx", col_types = c("date", 
    "numeric", "text"))
wahlen$Datum<-as.POSIXct.Date(wahlen$Datum)

weihnachtsmarkt <- read_excel("Weihnachtsmarkt.xlsx", 
    col_types = c("date", "numeric"))
weihnachtsmarkt$Datum<-as.POSIXct.Date(weihnachtsmarkt$Datum)

zeitumstellung <- read_excel("Zeitumstellung.xlsx", 
    col_types = c("date", "numeric", "text"))
zeitumstellung$Datum<-as.POSIXct.Date(zeitumstellung$Datum)
